...
const nhanVienSchema = Joi.object({
  ...
  vai_tro: Joi.string().valid('quan_ly', 'thu_ngan', 'nhan_vien_kho').default('quan_ly').messages({
  ...
  so_dien_thoai: Joi.string().pattern(/^[0-9]{10,11}$/).optional().allow(null, '').messages({
  ...
  trang_thai: Joi.string().valid('Dang_lam', 'Da_nghi').default('Dang_lam').messages({
...
router.put('/:id', authenticateToken, checkRole('quan_ly'), async (req, res) => {
  ...
  try {
    ...
    const nhanVien = await prisma.nhan_vien.findUnique({ where: { id: parseInt(id) } });
    if (!nhanVien) {
    ...
    const existingNhanVien = await prisma.nhan_vien.findFirst({
      where: {
        OR: [
          value.tai_khoan !== nhanVien.tai_khoan ? { tai_khoan: value.tai_khoan } : {},
          value.email && value.email !== nhanVien.email ? { email: value.email } : {},
        ],
        id: { not: parseInt(id) },
      },
    });
