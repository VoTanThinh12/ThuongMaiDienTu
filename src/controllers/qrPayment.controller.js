...
exports.createBankQRPayment = async (req, res) => {
  try {
    const { id } = req.params; // order id NUMBER
    const { customerId } = req;

    // Tìm đơn hàng theo ID number
    const order = await prisma.don_hang.findUnique({
      where: { id: Number(id) }, // ← ID là số
      include: {
        chi_tiet_don_hang: {
    ...
    // ✅ QUAN TRỌNG: Truyền ma_don_hang thay vì id
    const qrPayment = await qrPaymentService.createBankQRPayment(
      order.ma_don_hang, // ← SỬA: dùng ma_don_hang từ DB (VD: "DH001")
      order.tong_tien,
      `Thanh toan don hang ${order.ma_don_hang}`
    );
...
exports.handleBankWebhook = async (req, res) => {
  try {
    ...
    if (bankCode !== expectedBank || accountNumber !== expectedAccount) {
      console.log(
        `ℹ️ Transaction for different account: ${bankCode}-${accountNumber}`
      );
      return res.json({ success: true, ignored: true });
    }

    // 4) Tìm giao dịch pending phù hợp
    const matchedTransaction = await findMatchingPendingTransaction(
      amount,
      description
    );
    ...
    console.log(
      `✅ Processing payment for transaction: ${matchedTransaction.ma_giao_dich}`
    );
    ...
    console.log(
      `🎉 Payment confirmed successfully: ${matchedTransaction.ma_giao_dich}`
    );
...
exports.updateMoMoPaymentStatus = async (req, res) => {
  try {
    ...
    const transactionId = await qrPaymentService.updatePaymentStatus(
      orderId,
      "paid"
    );
...
function verifyWebhookSignature(body, headers) {
  ...
  if (!secret) {
    console.warn(
      "⚠️ No BANK_WEBHOOK_SECRET configured, skipping signature verification"
    );
  ...
  try {
    ...
    if (body.type && body.data) {
      ...
      return {
        ...
        description: String(
          data.description || data.content || data.transferNote || ""
        ),
        transactionRef: String(data.tid || data.transactionId || data.id),
        bankCode: String(
          data.accountBankCode || data.bankCode || "MB"
        ).toUpperCase(),
